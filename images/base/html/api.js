...
const topics = {
  ...
  "/vrx/wayfinding/waypoints": {
    onMsg: (msg) =>
      (data.poses = msg.poses.map((pose) => {
        ...
        eu.setFromQuaternion(ex);
        return {
          goal_pos: calcPolarCoords(
            pose.pose.position.latitude,
            pose.pose.position.longitude
          ),
          goal_vel: { r: 0, psi: eu.z },
          goal_global: {
            lat: pose.pose.position.latitude,
            lng: pose.pose.position.longitude,
          },
        };
      })),
  ...
  "/vrx/wildlife/animals/poses": {
    onMsg: (msg) =>
      (data.animals = msg.poses.map((pose) => {
        var eu = new three.Euler();
        var ex = new three.Quaternion(
          pose.pose.orientation.x,
          pose.pose.orientation.y,
  ...
  const actLoop = () => {
    if (getTaskInfo().state === "running") {
      var interval = setInterval(() => {
        craft.act();
        if (
          getTaskInfo().state === "finished" ||
          getTaskInfo().state === "Not started"
        ) {
 ...
 * @returns {Promise}
 */
const moveBackwards = (strength = 1) =>
  new Promise((resolve, reject) => {
    try {
      setLeftThrusterAngle(0);
      setRightThrusterAngle(0);
 ...
 * @returns {Promise}
 */
const rotateAnticlockwise = (strength = 1) =>
  new Promise((resolve, reject) => {
    try {
 ...
 * @returns {Promise}
 */
const rotateClockwise = (strength = 1) =>
  new Promise((resolve, reject) => {
    try {
...
const startSim = (type, serverUrl) => {
  if (sims.includes(type)) {
    ...
    return axios.post(`${serverUrl}/api/start_sim`, { sim: type });
  }
};
 ...
 */
const stopSim = (serverUrl) =>
  axios
    .post(`${serverUrl}/api/stop_sim`)
...
const calcPolarCoords = (lat, lng) => {
  ...
  var lengthLatDegreeInMetres =
    111132.92 -
    559.82 * Math.cos(2 * lat) +
    1.175 * Math.cos(4 * lat) -
    0.0023 * Math.cos(6 * lat);
  var lengthLngDegreeInMetres =
    (Math.PI / 180) * earthAvgRadius * Math.cos(lat);
